...
|                    |                                          | `R/.val`             | Table of charts, text          |
| `fetch_loc`        | Validate, convert and store input data   | `messages`           | parse messages                 |
|                    |                                          | `R/.val`             | `minihealth::individual` (int) |
| `draw_chart`       | Draw chart, predict future growth        | `graphics/1/svglite` | chart, `svglite` format        |
|                    |                                          | `graphics/1/png`     | chart, `png` format            |
|                    |                                          | `messages`           | parse, graph messages          |
|                    |                                          | `R/.val`             | `grid::gtree` object (int)     |
| `request_site`     | Create site URL with personalised charts | `R/.val/json`        | site URL, JSON                 |
|                    |                                          | `messages`           | parse, site messages           |
...
|                    |                                          | `messages`           | parse, screen messages         |
