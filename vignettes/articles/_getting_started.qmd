...
subtitle: JAMES 1.16.4 (August 2026)
author:
...
`/tanner/embed` is the Tanner-chart counterpart to `/charts/embed`: it returns a self-contained, interactive HTML document (a `plotly` chart) meant for embedding in an `<iframe>`. Unlike `/charts/embed`, there is exactly one chart -- no `chartcode`/multi-chart dispatch -- rendering genital/pubic hair/testicular volume for male patients, or breast/pubic hair/menarche for female patients, against the Dutch 1997 references (van Buuren, 2009, [PDF](https://stefvanbuuren.name/pdf/vanbuuren-2009-1.pdf)).
...
#### Reproducing van Buuren (2014), Figures 3 and 4

`/tanner/embed`'s `traces` and `stage_lines` parameters (comma-separated raw Tanner `varName`s: `gen`/`phb`/`tv` for male patients, `bre`/`phg`/`men` for female) control which stage types' patient-data traces and reference lines, respectively, are visible when the chart first renders -- without losing the ability to toggle any of them from the legend afterwards. The three figures below use them to reproduce van Buuren (2014)'s Figures 3 and 4. The chart itself is always rendered at `/tanner/embed`'s fixed 900x560 design resolution; to fit the article's column width, all three are scaled down 15% with a CSS `transform` on the iframe (the wrapping `<div>` reserves the post-scale footprint so surrounding content doesn't overlap it).
...
scaled_tanner_iframe <- function(srcdoc) {
  htmltools::tags$div(
    style = paste0("width: ", fig_width, "px; height: ", fig_height, "px;"),
    htmltools::tags$iframe(
      srcdoc = srcdoc,
      style = paste0(
        "width: 900px; height: 560px; border: 1px solid #ddd; ",
        "transform: scale(", scale_factor, "); transform-origin: top left;"
      )
    )
  )
}
...
##### Figure 1: Kevin_P, genital only, with stage lines (cf. Figure 3)
...
##### Figure 2: Kevin_P, all three traces, no stage lines (cf. Figure 4)
...
##### Figure 3: Iris_P, all three traces, no stage lines

Same layout as Figure 2, for a female patient: `Iris_P.json` (also from `jamesdemodata`) carries breast, pubic-hair, and menarche staging -- no reference lines.
