...
The site has an "Instellingen" panel where a user can pick the rendering engine (fixed/grid or interactive/plotly), toggle curve interpolation, and set the chart's display size. The `display` argument sets these before the user ever opens the panel, by appending extra query parameters (`?interactive=`, `?interpolation=`, `?size=`) to the site URL:
...
Paste the generated URL in your browser: the site opens with the interactive chart already selected, interpolation off, and the chart displayed at 600px. Any `display` element left unset keeps the site's own default (fixed engine, interpolation on, 785px).
