...
subtitle: JAMES 1.17.0 (August 2026)
author:
...
# Set target_host to one of:
# "internal" : Run only within the container
# "dev"      : localhost, development
# "test"     : james.groeigrammen.nl (default)
# "acc"      : acceptance server (authenticated)
...
hosts <- list(
  ...
  prod = "https://ijgz.eaglescience.nl/modules/james"
)
...
} else if (target_host == "prod") {
  # Try PROD with authentication, fall back to test if it fails
  auth_result <- system(
    "./authenticate-prod.sh",
    ignore.stdout = TRUE,
    ignore.stderr = TRUE
  )
  if (auth_result == 0) {
    host <- hosts$acc
    message("Using PROD server: ", host)
  } else {
    host <- hosts$test
    warning("PROD server unavailable, using fallback: ", host)
  }
} else if (target_host %in% names(hosts)) {
  host <- hosts[[target_host]]
  message("Using ", target_host, " server: ", host)
} else {
  stop("Invalid target_host. Choose: 'dev', 'test', 'acc' or 'prod'.")
}
...
# Detect the name of the current Quarto source file (e.g. "index.qmd")
doc <- knitr::current_input(dir = TRUE) # returns absolute path when available
if (is.null(doc)) {
  doc <- normalizePath("index.qmd", mustWork = FALSE) # fallback for interactive use
}
figdir <- file.path(
  dirname(normalizePath(doc, mustWork = FALSE)),
...
| POST  | `/charts/embed/{hwm}`      | Draw child data on an interactive chart    | `embed_chart()`          |
...
| &nbsp;| &nbsp;                     | &nbsp;                                     | &nbsp;                   |
| POST  | `/tab10/request/{dfm}`     | Request tab10 risk-communication session   | `request_tab10()`        |
| &nbsp;| &nbsp;                     | &nbsp;                                     | &nbsp;                   |
...
| `widget`   | Function result as interactive HTML document   |
...
| `hwm`        | Htmlwidget format: `widget`, `parent`                      |
...
if (target_host != "internal") {
  # print the host for verification
  cat("Target host code: ", target_host, "\n")
...
url <- modify_url(
  url,
  path = file.path(url$path, path),
  query = "auto_unbox=true"
)
r <- POST(
  url,
  add_headers(accept = "application/json", Authorization = auth_token)
)
...
```{bash bashversion, eval=!is_windows}
source auth_curl.sh
...
```{bash bashvalue1, eval=!is_windows}
source auth_curl.sh
...
```{bash vashvalue2, eval=!is_windows}
source auth_curl.sh
auth_curl -sX POST $(cat .host)/version/json?auto_unbox=true > value2
cat value2
```

```{bash removevalues, echo=FALSE, eval=!is_windows}
...
```{r}
success <- file.copy(
  system.file("examples/maria.json", package = "bdsreader"),
  "maria.json",
  overwrite = TRUE
)
```
...
```{r}
fn <- system.file(
  "extdata/bds_v3.0/smocc/Laura_S.json",
...
For square charts, use query arguments `list(height = 18/2.54, width = 18/2.54)` to generate the plot.

```{r}
#| echo: false
htmltools::HTML(r7$parsed)
```
...
### **`/charts/embed`**: Draw child data on an interactive growth chart

`/charts/embed` is the interactive counterpart to `/charts/draw`: instead of an SVG image, it returns a self-contained, interactive HTML document (a `plotly` chart) meant for embedding in an `<iframe>` -- not an `<img>`, since it is not an image. It supports the same donor-matching, prediction and target-height features as `/charts/draw`.

Unlike `/charts/draw`, `/charts/embed` cannot return the chart directly from the `POST` request: the response is always an OpenCPU session -- a listing of URLs, such as:

```
/ocpu/tmp/x0ad0c0287a0185/R/.val
/ocpu/tmp/x0ad0c0287a0185/R/embed_chart
/ocpu/tmp/x0ad0c0287a0185/warnings
/ocpu/tmp/x0ad0c0287a0185/source
/ocpu/tmp/x0ad0c0287a0185/console
/ocpu/tmp/x0ad0c0287a0185/info
/ocpu/tmp/x0ad0c0287a0185/files/DESCRIPTION
```

Fetching the chart itself is a second, separate `GET` request: append `/widget` to the session key (the first path segment, `x0ad0c0287a0185` above).

::: panel-tabset

##### **R**

We can draw Laura's data on an interactive chart. `james_post()`'s return value carries the session key in `$session`:
...
Fetching a sub-resource of an existing session is a `GET` request, not a `POST`, so use `httr::GET()` directly to retrieve the rendered chart:

```{r}
widget_url <- paste0(host, "/", r8$session, "/widget")
r8_widget <- httr::content(httr::GET(widget_url), "text", encoding = "UTF-8")
```

The response is the interactive HTML document itself, ready to be written to a file (`writeLines(r8_widget, "laura.html")`) or embedded directly. It references plotly.js and jQuery from a public CDN rather than inlining them:

```{r}
nchar(r8_widget) # bytes, with plotly.js/jQuery loaded from a CDN
```

For comparison, a fully self-contained version of the same chart (everything inlined, no CDN needed) is much larger: about 1.2MB with unused plotly.js modules stripped out, or close to 4MB with the full plotly.js bundle.

The chart is rendered at a fixed 900x900px design resolution. Embedded directly (e.g. `<iframe src="laura.html">`), it fills the available width and scrolls if the container is narrower than 900px:
...
To scale it down to a specific size instead of scrolling -- say 600x600px -- wrap the `<iframe>` in an outer div sized to the target, with the iframe itself kept at the full 900x900px design resolution and shrunk uniformly with CSS `transform: scale()`. This keeps text, markers and the logo in their correct proportions, unlike letting the browser reflow a resized `<iframe>`'s contents (which it can't do anyway, since an `<iframe>`'s contents are laid out independently of its own size):

```{r}
design_side <- 900
display_side <- 600
scale <- display_side / design_side

htmltools::div(
  style = sprintf(
    "width: %1$dpx; height: %1$dpx; overflow: hidden; position: relative;",
    display_side
  ),
  htmltools::tags$iframe(
    srcdoc = r8_widget,
    style = sprintf(
      "width: %1$dpx; height: %1$dpx; border: 1px solid #ddd; transform: scale(%2$f); transform-origin: top left;",
      design_side,
      scale
    )
  )
...
To verify a saved file works standalone, open it directly in a browser (e.g. `browseURL("laura.html")`) -- the chart should render at its full 900x900px size and respond to hovering over data points with tooltips.

##### **bash**

```{bash, eval=!is_windows}
source auth_curl.sh
auth_curl -sX 'POST' $(cat .host)'/charts/embed' \
-H 'accept: text/plain' \
-F "chartcode='NMBH'" \
-F "selector='chartcode'" \
...
Extract the session key from the first line of the response and fetch `/widget` from it:

```{bash, eval=!is_windows}
source auth_curl.sh
SESSION=$(auth_curl -sX 'POST' $(cat .host)'/charts/embed' \
-H 'accept: text/plain' \
-F "chartcode='NMBH'" \
-F "selector='chartcode'" \
-F 'txt=@maria.json;type=application/json' \
| head -1 | sed -E 's|.*/(x0[a-z0-9]+)/.*|\1|')
auth_curl -sX 'GET' $(cat .host)'/'$SESSION'/widget' > maria.html
wc -c maria.html
```

The resulting `maria.html` is a self-contained document (plotly.js and jQuery are loaded from a public CDN, so viewing it requires an internet connection, but keeps the file well under 250KB -- see the **R** tab for a size comparison against a fully self-contained version, over 1MB) and can be embedded in another page with:

```html
<iframe src="maria.html"></iframe>
```

To check the round trip worked, open `maria.html` directly in a browser (e.g. `open maria.html` on macOS, `xdg-open maria.html` on Linux, or double-click it in a file browser) -- the chart should render and respond to hovering over data points with tooltips. Loading plotly.js and jQuery from a CDN keeps the file small (typically well under 250KB, vs. over 1MB fully self-contained), so it's practical to send as a normal HTTP response body rather than writing it to disk first. Since the interactivity is all client-side JavaScript operating on data already embedded in the file, the chart works the same way when hosted as a static file (e.g. on GitHub Pages) as it does opened locally -- only the two CDN scripts need a live internet connection, not the hosting server itself.

:::

### **`/tanner/embed`**: Draw an interactive Tanner pubertal-stage chart

The Tanner stages of puberty are measured differently by sex: for boys, genital development (`gen`, G1-G5, 5 stages), pubic hair development (`phb`, P1-P5, 5 stages) and testicular volume (`tv`, continuous in ml); for girls, breast development (`bre`, B1-B5, 5 stages), pubic hair development (`phg`, P1-P5, 5 stages) and menarche (yes/no, 2 stages). These stages are typically referenced as age centiles -- the age at which the reference population has passed a given stage. The stage-line chart (van Buuren and Ooms, 2009, [PDF](https://stefvanbuuren.name/pdf/vanbuuren-2009-1.pdf)) reverses that direction of inference, presenting centiles of stage at a given age instead, much like the usual evaluation of height and weight.

`/tanner/embed` is the Tanner-chart counterpart to `/charts/embed`: it returns a self-contained, interactive HTML document (a `plotly` chart) meant for embedding in an `<iframe>`. Unlike `/charts/embed`, there is exactly one chart -- no `chartcode`/multi-chart dispatch -- rendering genital/pubic hair/testicular volume for male patients, or breast/pubic hair/menarche for female patients, against the Dutch 1997 references.

When the uploaded data yields no target, or the target has no pubertal measurements at all, the chart still renders -- just the reference frame, with no patient trace -- rather than the call failing. `sex` (`"male"`/`"female"`) is optional and overrides the sex derived from the data's own persondata; omit it to let the data decide. Like `/charts/embed`, the response is always an OpenCPU session -- fetch the widget itself with a second, separate `GET` request on `/{session}/widget`.

`/tanner/embed`'s `traces` and `stage_lines` parameters (comma-separated raw Tanner `varName`s: `gen`/`phb`/`tv` for male patients, `bre`/`phg`/`men` for female) control which stage types' patient-data traces and reference lines, respectively, are visible when the chart first renders -- without losing the ability to toggle any of them from the legend afterwards. The chart itself is always rendered at `/tanner/embed`'s fixed 900x560 design resolution; to fit the article's column width, all three are scaled down 15% with a CSS `transform` on the iframe (the wrapping `<div>` reserves the post-scale footprint so surrounding content doesn't overlap it).

```{r}
scale_factor <- 0.85
fig_width <- round(900 * scale_factor)
fig_height <- round(560 * scale_factor)
scaled_tanner_iframe <- function(srcdoc) {
  htmltools::tags$div(
    style = paste0(
      "width: ",
      fig_width,
    ...
    htmltools::tags$iframe(
      srcdoc = srcdoc,
      style = paste0(
        "display: block; width: 900px; height: 560px; border: 1px solid #ddd; ",
        "transform: scale(",
        scale_factor,
...
`Kevin_P.json` (from `jamesdemodata`, male; genital, pubic hair, testicular volume) is used for Figures 1 and 2:

```{r pubertydemo}
fn_kevin <- system.file(
  "extdata/bds_v3.1/puberty/Kevin_P.json",
  package = "jamesdemodata"
)
```

#### Example 1: Genital only, with stage lines

Only the genital trace is shown, with its reference/stage lines visible; pubic hair and testis are left at `/tanner/embed`'s own default (visible, no lines) since `traces` here only lists `gen`.
...
widget_url10 <- paste0(host, "/", r10$session, "/widget")
r10_widget <- httr::content(httr::GET(widget_url10), "text", encoding = "UTF-8")
```

```{r}
#| echo: false
scaled_tanner_iframe(r10_widget)
```

Interpretation: Genital development is normal for this boy. Between ages 12.13 and 12.36 years, a transition from G1 to G2 occurs,  followed by a transition to G3 at age 13.13. No transition to the last category G5 was registered.

#### Example 2: Boy, all three traces, no stage lines

All three stage types' patient data shown; no reference lines -- `/tanner/embed`'s own default when `traces`/`stage_lines` are omitted, made explicit here for clarity.
...
widget_url11 <- paste0(host, "/", r11$session, "/widget")
r11_widget <- httr::content(httr::GET(widget_url11), "text", encoding = "UTF-8")
```

```{r}
#| echo: false
scaled_tanner_iframe(r11_widget)
```

Interpretation: The same boy had very early public hair development, even reaching into the 1% early group for P3 at age 9.86 years. In contrast, the development of testical volume was quite normal.

#### Example 3: Girl, all three traces, no stage lines

Same layout as example 2, for a female patient: `Iris_P.json` (also from `jamesdemodata`) carries breast, pubic-hair, and menarche staging -- no reference lines.

```{r pubertyfig3}
fn_iris <- system.file(
  "extdata/bds_v3.1/puberty/Iris_P.json",
  package = "jamesdemodata"
)
r12 <- james_post(
  host = host,
...
widget_url12 <- paste0(host, "/", r12$session, "/widget")
r12_widget <- httr::content(httr::GET(widget_url12), "text", encoding = "UTF-8")
```

```{r}
#| echo: false
scaled_tanner_iframe(r12_widget)
```

Interpretation: This girls is late-maturing on all aspects. Menarche occurs between ages 15.25 and 15.76 years. Breast and pubic hair development was also delayed.
...
Body version, file to string. Missing data in the results replaced by `null` fields.
...
The site has an "Instellingen" panel where a user can pick the rendering engine (fixed/grid or interactive/plotly), toggle curve interpolation, and set the interactive chart's display size (the fixed/grid chart keeps its own fixed sizes, since its server-rendered SVG text doesn't scale with the requested width/height). The `display` argument sets these before the user ever opens the panel, by appending extra query parameters (`?interactive=`, `?interpolation=`, `?size=`) to the site URL:
...
Paste the generated URL in your browser: the site opens with the interactive chart already selected, interpolation off, and the interactive chart displayed at 600px. Any `display` element left unset keeps the site's own default (fixed engine, interpolation on, 785px interactive display size).
...
### **`/tab10/request`**: Request tab10 session

::: panel-tabset

##### **R**

The `/tab10/request` end point uploads child data and creates a URL to a standalone `tab10` Shiny app for personalised risk communication ("Tafel van Tien"). Like `/site/request`, it accepts either a raw `txt` (uploads the data itself) or an existing `session` (reuses data already uploaded, e.g. by a prior `/site/request` or `/data/upload` call, without a redundant re-upload):

```{r tab10request1}
r <- james_post(
  host = host,
  path = "/tab10/request/json",
  sitehost = host,
  txt = js
)
r$parsed
...
Paste the generated URL in the address field of your browser to open tab10 with the uploaded data already loaded.

Reusing a session from a prior upload avoids uploading the same data twice -- useful when a caller (e.g. `jamesapp`) already holds a session from its own `/site/request` call:
...
files <- c("laura.svg", "maria.json", "maria1.svg", "maria2.svg", "result.json")
file.remove(files)
```
...
```r
target_host <- "internal" # localhost:80, run this qmd at the container start
target_host <- "dev"   # localhost:8080 for development
