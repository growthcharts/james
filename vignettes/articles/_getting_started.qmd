...
We can draw Laura's data on an interactive chart in one step:

```{r}
r8 <- james_post(
  host = host,
  path = "/charts/embed/widget",
  txt = fn,
  chartcode = "NMBA",
  selector = "chartcode"
)
...
```{bash, eval=!is_windows}
source auth_curl.sh
auth_curl -sX 'POST' $(cat .host)'/charts/embed/widget' \
-H 'accept: text/html' \
-F "chartcode='NMBA'" \
-F "selector='chartcode'" \
-F 'txt=@maria.json;type=application/json' > maria.html
```
