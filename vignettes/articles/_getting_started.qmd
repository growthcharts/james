...
The response is the interactive HTML document itself, ready to be written to a file or embedded directly:

```{r}
#| echo: false
htmltools::HTML(r8_widget)
```
...
The resulting `maria.html` is a self-contained document (plotly.js and jQuery are loaded from a public CDN, so viewing it requires an internet connection) and can be embedded in another page with:
