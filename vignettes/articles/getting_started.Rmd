...
subtitle: JAMES 1.7.3.9001 (August 2025)
...
By specifying the instrument we can specify whether we want to include `ddi`, `gs1` or items from both (default is `gs1`). We must additionally specify a key that supports both instruments.

```{r}
fn <- system.file("examples", "example_v3.1.json", package = "bdsreader")
r <- james_post(host = host, path = "dcat/calculate/json", txt = fn, instrument = c("ddi"), key = "gsed2406")
r
head(r$parsed)
```
...
The above command fails for `localhost:8080`, and gives the error  `[400] Couldn't connect to server [localhost]`, because `localhost:8080` does not allow nested requests to itself.

The solution is the break the request into two steps. First, we create a session by uploading the child data, and then we use that session to create the URL to the personalised site.
...
As before, the following command only works on remote servers, not on `localhost:8080`.
