...
The D-score is in column named `y` and the DAZ is in column `z`.

##### **bash**

```{bash eval=TRUE}
HOST=$(tr -d '\n' < .host)
curl -sS -X POST "$HOST/dscore/calculate/json" \
  -H 'Content-Type: application/json' \
  --data-binary @- <<'EOF'
{"txt":"https://raw.githubusercontent.com/growthcharts/jamesdemodata/HEAD/inst/extdata/bds_v3.0/smocc/Laura_S.json","output":"last_visit"}
EOF
```
