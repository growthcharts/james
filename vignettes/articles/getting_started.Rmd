...
This document provides a quick introduction into the main JAMES features, and how these can be assessed from `R` and from the command line. The example requests illustrate the use of JAMES API 1.2.0. The service
...
JAMES is a RESTful API that runs on a remote host. In principle, any `HTTP` client will work with JAMES. The document highlights some applications of the service and provides pointers to relevant background information. The current OpenAPI definition of JAMES is at <https://app.swaggerhub.com/apis-docs/stefvanbuuren/james> (version 1.2.0, April 2022). Note that this definition may evolve over time.
...
cat(getwd(), "\n")
writeLines(r5$parsed, con = "figures/chart1.svg")
```
...
r6 <- james_post(host = host,
                 ...
                 query = list(height = 29.7/2.54, width = 21/2.54))
writeLines(r6$parsed, con = "figures/chart2.svg")
```
...
writeLines(r7$parsed, con = "figures/chart3.svg")
```
